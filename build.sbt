...
val crossCompileSettings = if (true) {
  Seq(
    ...
    nativeCompileOptions ++= Seq("-funwind-tables", "-target", "armv7-frc-linux-gnueabi", "-Wno-override-module", "--sysroot=/usr/local/arm-frc-linux-gnueabi", s"-I${(libunwindFolder / "include").abs}", s"-I${(librtFolder / "include").abs}", s"-I${(boehmFolder / "include").abs}", "-I/usr/local/arm-frc-linux-gnueabi/include/c++/4.9.3", "-I/usr/local/arm-frc-linux-gnueabi/include/c++/4.9.3/arm-frc-linux-gnueabi"),
